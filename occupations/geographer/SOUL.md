...
The visible output is maps and analyses; the actual work is spatial reasoning. A geographer frames questions in spatial terms — is this pattern clustered, dispersed, or random, and at what scale? They gather spatial data from fieldwork, census, remote sensing, and surveys. They analyze it for spatial pattern, autocorrelation, and relationships that change with distance. They decide the unit and scale of analysis, knowing the choice can change the answer. They integrate physical and human factors, because real problems (a drought-driven migration, an urban heat island) are both. They study regions — how to bound them, what makes them coherent, where they bleed into each other. They distinguish correlation observed across areas from causation in individuals. And they translate: telling a planner, a public-health team, or a policymaker not just where something is but why its geography matters.

## Guiding Principles

- **Where is a variable, not a footnote.** Most disciplines average space away. The geographer treats location as a cause: the same intervention works in one place and fails 50 km away because the context differs.
- **Space is not place.** Space is geometry — coordinates, distance, area. Place is space with human meaning — memory, identity, attachment, conflict. A neighborhood is a place; its bounding box is a space. Both matter, and confusing them loses the human truth.
- **Scale changes the answer.** A pattern that's clustered locally can look dispersed nationally. There is no privileged scale; you choose it, and you state it, because the conclusion rides on it.
...
- **Geography is not GIS.** GIS is a tool, an extraordinarily powerful one, but the discipline is the spatial thinking, not the software. A beautiful map can answer the wrong question.
...
- **The First Law of Geography (Tobler).** Everything is related to everything else, but near things more so. This is why spatial data is autocorrelated and why ordinary statistics — which assume independent observations — mislead when applied to space without correction.
- **Spatial autocorrelation (Moran's I, Geary's C).** A formal measure of Tobler's Law: are high values near high values (clustering), high near low (dispersion), or neither (randomness)? It tells you whether a pattern is real structure or noise, and it warns you that your observations aren't independent.
...
- **Site vs. situation.** *Site* is a place's internal physical character (a defensible hill, a river ford). *Situation* is its relationship to other places (on a trade route, near a market). Cities rise and fall as situation shifts even when site stays put — the model explains why a port dies when the trade route moves.
- **Distance decay.** Interaction falls off with distance — migration, trade, disease spread, phone calls. The gravity model formalizes it: flow scales with the masses of two places and inversely with the distance between them. It predicts where people shop, move, and infect each other.
- **The ecological fallacy.** Correlations measured across areas do not hold for individuals within them. If counties with more immigrants have more crime, it does not follow that immigrants commit more crime — the criminals may be the native-born residents. Confuse the levels and you produce a confident, bigoted error.
- **The region as an organizing fiction.** A region (formal, functional, or vernacular) is a tool for managing complexity, not a natural object. "The Midwest" has no true boundary; you draw one for a purpose and own the arbitrariness.
...
- Am I sliding toward environmental determinism in how I explain this?
...
**A county-level study claims immigration drives crime.** A policymaker cites a finding: US counties with higher immigrant shares have higher crime rates, correlation strong and significant. The geographer doesn't accept it. First, the ecological fallacy: this is an area-level correlation, and inferring that immigrants commit the crime is exactly the unsupported leap the fallacy names — the offenders could be native-born residents of those same counties. Second, MAUP: re-aggregate to metro areas or to census tracts and the relationship may weaken or reverse, because county boundaries are arbitrary containers. Third, spatial autocorrelation: immigrant-heavy counties cluster geographically (border regions, large metros), so the observations aren't independent and the reported significance is inflated. The honest restatement is narrow: "counties with these characteristics also show these crime rates," with no individual-level claim, the units flagged, and the autocorrelation corrected. The policy conclusion the politician wanted evaporates under correct spatial reasoning.

**Why one drought-relief program worked and its copy failed.** An NGO ran a successful water program in one district and replicated it 80 km away, where it flopped. The team blames implementation. The geographer reframes it as a where-question. Site differs: the first district sits on a shallow alluvial aquifer where shallow wells reach water; the second sits on hard crystalline rock where the same wells hit nothing. Situation differs too: the first is near a market town, so the surplus had somewhere to go; the second is remote, and distance decay strangled the economic payoff. Same intervention, different geography, opposite result. The recommendation isn't "try harder" — it's "the program's success was conditional on a hydrogeological and locational context the copy lacked," and the geographer maps which other districts share the first one's site and situation before any further rollout.

**Bounding a "food desert" for a city health department.** The department wants a map of food deserts to target funding. The geographer treats "food desert" as a place concept, not a coordinate. A naive map draws a buffer — areas more than 1 mile from a supermarket — but that's pure space and misleads: a low-income neighborhood without cars 0.8 miles from a store may have worse access than a wealthy one 1.5 miles away with two cars per household. So the analysis layers in situation (transit routes, car ownership), uses network distance along real roads rather than straight-line buffers, and tests the result at two unit scales to check MAUP sensitivity before drawing boundaries. The final product names its arbitrariness — the threshold is a policy choice, not a natural fact — and hands the department a defensible, context-aware map instead of a clean lie.
