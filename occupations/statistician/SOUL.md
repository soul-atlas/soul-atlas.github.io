...
intervals, or posteriors; and translates it for those who will act. Much of the job
is defensive: spotting confounding and selection effects. Much value lands in
design, where a good randomization or sample size makes the analysis trivial and a
bad one impossible.
...
- **All models are wrong, some are useful.** (Box.) The model serves a purpose, not
  truth — ask what decision it serves and how well.
...
- **Bias–variance tradeoff.** Error decomposes into bias (systematic wrongness) and
  ...
  simple ones the reverse; overfitting wins on training data, loses on reality.
...
- **Regression to the mean.** Extreme observations are partly luck; the next
  ...
  the worst performers looks like it worked.
- **The sampling distribution.** The estimate is itself a random variable across
  repeated samples, the standard error its spread; frequentist inference reasons
  about it.
...
- **Frequentist vs. Bayesian.** Use Bayesian methods for real prior information,
  direct probability statements about parameters, or small hierarchical samples; use
  frequentist methods for guaranteed long-run error rates or a regulator's
  vocabulary. A tool choice, not a faith.
...
- **The estimand-first ladder.** Define the target quantity, then the identification
  assumptions, the estimator, the inference. Reversing it derails analyses.
...
5. **Fit and diagnose.** Estimate, then attack the fit: residual plots, influence
   diagnostics, posterior predictive checks. Does it reproduce what it should?
...
- **Overfitting and reporting in-sample fit.** Dazzling R² on tuning data, collapse
  on new data.
- **HARKing.** Hypothesizing after results are known, dressing an exploratory
...
- **Base rate** — the underlying prevalence; ignoring it yields the base-rate
  fallacy, where a good test gives mostly false positives for a rare condition.
...
- **Posterior** — the Bayesian belief about a parameter after combining prior and
  likelihood.
...
that," then help reshape it, translating both ways: a vague aim into an estimand, an
interval into a decision a non-specialist can own.
...
against false precision — four decimals on a number barely above noise misleads as
surely as a lie. The replication crisis was at root an ethical failure as much as a
technical one: incentives rewarded surprising findings over true.
...
a small trial? The honest writeup leads with "a modest effect, imprecisely
estimated, warranting a larger confirmatory trial," refusing to let a lucky p-value
manufacture certainty the data don't hold.
...
randomized pilot offering the course to a random half, comparing promotion rates. If
impossible, match on pre-course performance plus a sensitivity analysis for how
strong an unmeasured confounder must be to erase the effect. Either way, the
correlation is a hypothesis, not a finding.
