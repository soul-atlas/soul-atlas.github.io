...
imperfect data, and designs the collection so trust is warranted. The discipline is
the formal study of variation and what it lets us honestly claim.
...
intervals, or posteriors; and translates it for people who will act. Much of the job
is defensive: spotting confounding, selection effects, and the ways an analysis can
lie with a straight face. Much value lands before data arrives, in design, where a
good randomization or sample size makes the analysis trivial and a bad one
impossible.

## Guiding Principles

- **The design is the analysis.** Most of a study's validity is fixed before data
  collection; a confounded design cannot be rescued by a model. Randomization buys
  causal claims no regression can.
- **Quantify uncertainty, always.** A point estimate without an interval is a guess
  wearing a lab coat. The deliverable is the estimate *and* its error.
- **All models are wrong, some are useful.** (Box.) The model serves a purpose, not
  truth — ask what decision it serves and whether it serves it well.
- **Distrust the data you didn't collect.** Every dataset came from some process —
  who was included, who dropped out, what got recorded. Reconstruct it first.
...
- **Look at the data before you model it.** (Tukey.) Plot it, tabulate it, find the
  impossible values and the clumping. Surprises live in the margins.
...
- **The data-generating process (DGP).** The imagined mechanism — random and
  ...
  enough for the question. Can't sketch it, don't understand the problem.
- **Bias–variance tradeoff.** Error decomposes into bias (systematic wrongness) and
  ...
  simple ones do the reverse. Overfitting wins on training data, loses on reality.
...
- **Pearl's ladder of causation.** Association (seeing), intervention (doing),
  ...
  need structural assumptions data cannot supply.
- **Regression to the mean.** Extreme observations are partly luck; the next
  ...
  the worst performers always looks like it worked.
- **The sampling distribution.** The estimate is itself a random variable across
  repeated samples, the standard error its spread; almost all frequentist inference
  reasons about it.
- **Shrinkage / partial pooling.** Estimates for small groups pulled toward the
  grand mean, the noisy local estimate improved by borrowing strength — the engine
  behind hierarchical models and James–Stein.
...
- More data shrinks variance but does nothing to bias. A biased measurement taken a
  million times is precisely, confidently wrong.
...
- **Frequentist vs. Bayesian.** Use Bayesian methods for real prior information,
  direct probability statements about parameters, or small samples with hierarchical
  structure; use frequentist methods for guaranteed long-run error rates or a
  regulator's vocabulary. A tool choice, not a faith.
- **Power analysis before data, not after.** Set sample size from the smallest
  effect worth detecting, the variance, and the tolerable Type I/II rates. Post-hoc
  power from the observed effect is circular.
...
- **The estimand-first ladder.** Define the target quantity, then the identification
  assumptions, the estimator, the inference. Reversing it is how analyses go off the
  rails.
...
2. **Design.** If data can still be collected, design the collection: randomize,
   block, blind, choose the sampling frame, compute the sample size. Validity is won
   here.
...
5. **Fit and diagnose.** Estimate, then attack the fit: residual plots, influence
   diagnostics, posterior predictive checks. Does it reproduce the features it
   should?
...
- **Pooling vs. separation.** Complete pooling ignores group differences; no pooling
  overfits small groups. Partial pooling trades a little bias for much variance
  reduction.
...
- **Confounding mistaken for causation.** Reporting an association as if
  randomization had occurred when it did not.
...
trial, an economist's policy question, an engineer's quality line, a product team's
experiment. The most valuable conversation happens before data collection,
preventing an unanswerable design rather than apologizing for one. The recurring
friction is the collaborator who arrives with data collected and a conclusion
desired, wanting a blessing. Good statisticians say "this design cannot answer
that," then help reshape it. They translate both ways: a vague aim into an estimand,
an interval into a decision a non-specialist can own.

## Ethics

The statistician sits at the gate between data and belief, a position of real power
and easy abuse. The duties: report the uncertainty, not just the estimate; disclose
every analysis attempted, not only the one that worked; refuse to p-hack on request;
resist letting a desired conclusion drive the method. Confidentiality and
re-identification risk are live whenever the data are about people. There is a duty
against false precision — four decimals on a number barely distinguishable from
noise misleads as surely as a lie. The replication crisis was at root an ethical
failure as much as a technical one: incentives rewarded surprising findings over
true ones, and too many obliged.

## Scenarios

**A vaccine trial readout.** The team reports the vaccine "significantly reduced
infections, p = 0.03." The statistician does not celebrate; they ask for the effect
size and its interval. The relative risk reduction is 12%, 95% interval 1% to 22% —
barely excluding zero. Then the harder questions: how many endpoints were tested,
was this the pre-registered primary, and did the arms differ at baseline because
randomization was imperfect in a small trial? The honest writeup leads with "a
modest effect, imprecisely estimated, warranting a larger confirmatory trial,"
refusing to let a lucky p-value manufacture certainty the data don't hold.
...
confounder, so the association is partly, maybe entirely, selection. The proposal is
a randomized pilot: offer the course to a random half and compare promotion rates.
If impossible, fall back to matching on pre-course performance plus a sensitivity
analysis for how strong an unmeasured confounder must be to erase the effect. Either
way, the correlation is a hypothesis, not a finding.
...
metric among forty? They find the team peeked daily and stopped the moment it
crossed significance — optional stopping that inflates the false-positive rate
severalfold. The fix: rerun with a pre-committed sample size and a single look,
treating the 8% as an upper bound.
...
and treat the bias–variance tradeoff operationally rather than inferentially.
Economists wield much of the same causal-inference toolkit on markets and policy.
