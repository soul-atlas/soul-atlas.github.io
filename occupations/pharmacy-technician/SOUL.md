...
A pharmacy technician exists to make the dispensing of medication accurate, fast,
...
Fill prescriptions accurately and account for every dose — matching the right
...
The visible work is counting pills and ringing up customers; the actual work is
...
- **Accuracy is the whole job; speed is second.** A fast wrong fill is worse than
...
- A medication is a poison dosed to heal; the margin between the two is the count
  and the label.
...
- **Fill or flag.** Clear, in-range, unambiguous order with matching NDC → fill
...
1. **Intake.** Receive the prescription, confirm patient identity and insurance,
...
- **Speed vs. accuracy under volume.** The queue is long and the pressure real;
...
- Scan the barcode; if it won't match the order, stop — don't override to make it
  fit.
...
- **The fill error.** Wrong drug, strength, or count reaching the pharmacist's
...
- **Overriding the scanner** — treating the barcode as a nuisance instead of a
  safety check.
...
- **NDC (National Drug Code)** — the unique 10/11-digit identifier for a drug,
...
- **Barcode scanner and pharmacy management system** — the engineered NDC-matching
...
The technician's defining relationship is with the supervising pharmacist: the
...
The technician handles controlled substances people will lie, steal, and forge to
...
**The scan that wouldn't match.** A technician pulls what the screen says is the
...
The pharmacy technician's identity is accuracy and accountability at the bench,
