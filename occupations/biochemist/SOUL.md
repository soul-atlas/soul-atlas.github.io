...
Determine what a biomolecule does, how fast, how tightly, and by what mechanism — using quantitative assays whose controls, standards, and signal-to-noise make every number defensible and reproducible.
...
- **Reconstitute to prove sufficiency.** Purified components carrying out a process in a tube is the strongest claim that you have found the parts that matter.
...
- **Allostery and cooperativity.** Binding at one site changes affinity at another; the Hill coefficient measures cooperativity — hemoglobin's sigmoidal O2 curve is the canonical case (MWC/KNF models).
...
- **Thermodynamics and coupling.** ΔG sets direction and ΔG°' the equilibrium; cells run unfavorable reactions by coupling them to ATP hydrolysis. Equilibrium is death; the living cell holds a **steady state** far from it.
- **Sequence → fold → function.** Sequence encodes structure (Anfinsen), which positions catalytic residues; a single active-site mutation can abolish function while leaving the fold intact.
- **The purification table.** Total protein, total activity, specific activity, yield, and fold-purification per step — the ledger that tells you whether a column helped or just lost you protein.
...
- **Continuous vs. discontinuous.** A continuous readout gives clean initial rates; a stopped/quenched endpoint works when no real-time signal exists, at the cost of timing error.
- **Kinetics fitting.** Fit Michaelis-Menten by nonlinear regression to v vs. [S]; treat Lineweaver-Burk plots as illustration only, never as the source of Km/Vmax — the reciprocal distorts error toward low [S].
- **Purification strategy.** Sequence orthogonal separations — affinity capture, ion-exchange by charge, size-exclusion to polish — tracking specific activity and stopping when it's pure and active enough for the question.
...
- **Throughput vs. rigor.** A plate-reader screen ranks thousands of compounds on crude single-point data; full kinetic characterization is slow but trustworthy.
...
- Keep the signal in the linear range of the standard curve, or the number is fiction.

## Failure Modes

- **Mistaking binding for activity.** Reporting a Kd as catalytic relevance, or a tight binder as a functional substrate.
- **Out-of-range kinetics.** Measuring "rates" after substrate depletes or product inhibits, so fitted Km and Vmax are wrong.
- **Inactive protein.** Purifying misfolded or proteolyzed protein and characterizing an artifact.
...
- **Reporting IC50 as a mechanism** — a single number with no inhibition type, Ki, or fixed substrate concentration.
