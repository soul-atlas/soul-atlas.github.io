...
The copy is written, but the read must sound spontaneous; the clock is merciless,
but the delivery must sound unhurried. The craft is hiding the machinery so
...
— often as a one-person operation. They hit the post: the last word of a talk-up
landing precisely as a song's vocal begins, to the half-second. They work to a
clock, sequencing segues, beds, stings, and breaks so the hour ends on time for
the network join or top-of-hour ID. They ad-lib inside a rigid format, take
direction through the IFB/talkback while still speaking, identify sponsors
correctly, run EAS emergency alerts when they fire, and never let dead air
happen. Above all they protect the license by staying inside FCC limits every
second the mic is hot.

## Guiding Principles

- **Talk to one person.** The audience is thousands, but each listener is alone
  in a car or a kitchen. "Hey everybody" is dead air with words.
- **The read must sound like the thought just occurred to you.** Polished copy
  delivered like copy is amateur. Mark it up, internalize it, then throw the page
  away in your head and just say it.
...
- **Serve the format, then find room inside it.** The clock and the brand voice
  aren't enemies; they're the frame. Personality is what you do in the gaps they
  leave.
...
- **Voice as an instrument.** Pitch, pace, volume, inflection, and breath are
  ...
  pitch — tuned with the diaphragm, not the throat.
...
- **The post as a landing target.** Every song has an intro ramp before the
  vocal; the post is the last instant you can still be talking. You time the
...
- **Format clock and brand voice.** The station is a product with a defined sound
  — the words, the topics, the energy band. You are a member of a cast playing a
  consistent character the listener tuned in to hear.
...
free — it is conditioned on serving the public interest.
...
open (vocal from zero), stay out. When in doubt, button it shorter and end early
rather than crash the post.
...
a weather hit, a "coming up" tease — and let the control room solve it off-mic.
...
through the IFB without breaking stride. Watch the clock continuously, trimming
or stretching the talk to land on the network join to the second. Run EAS the
instant it fires. Between breaks, line up the next element, scan ahead for
trouble, breathe. Close the mic. Log what aired, note any make-goods owed to a
sponsor, hand off cleanly to the next jock or the network.

## Common Tradeoffs

- **Spontaneity vs. control.** The loosest, funniest read is the riskiest one.
  You trade a little freedom for the guarantee you'll hit the post and stay
  legal.
...
Forgetting the top-of-hour ID, a flat FCC requirement, not a style choice.
...
- **The walk-over** — talking through the vocal because you didn't check the
  intro.
- **The apology spiral** — narrating every flub and dead spot, doubling the
  damage.
...
- **IFB / talkback:** the producer's voice fed into the announcer's ear on-air
...
NexGen) that holds the music, spots, and clock. A profanity delay unit, the EAS
encoder/decoder, and the IFB/talkback link to the producer. A visible studio
clock and a countdown to the network join. Marked-up paper or on-screen copy.
For sports PA and play-by-play: a spotter board, roster, and stat sheet.
...
two operate as one nervous system. The program director owns the format and
sound the announcer must serve. Traffic schedules the spots and owns the
make-goods. The news and weather voices hand off in tight, choreographed
transitions. Sales sets the sponsor reads; engineering keeps the signal and the
delay alive. In sports, the play-by-play and color commentator trade on a
rhythm; the analyst, spotter, and truck's director keep the call synced to the
replay. A clean handoff is invisible; a fumbled one is dead air for everyone.
...
exist partly to protect children; the line is real and the announcer holds it.
The voice carries authority, so honesty matters — don't read a lie as news,
don't blur an ad into editorial. In live moments, the power to inflame or calm a
crowd or a region is real; the trusted voice uses it to steady, not to
stampede.

## Scenarios

**The cold open that almost crashed the post.** A jock cues the next song and
glances at the screen: intro time reads :08, but it's a cold open — the vocal
starts at zero. The temptation is to start the planned talk-up over the top.
Instead the read is instant: no ramp. The jock back-announces the previous song
over the outro, lets the new song hit clean, and saves the personality break for
the next song, which has a generous :19 ramp. The line lands exactly on the post
there, and nobody at home heard a decision get made — that's the point. When the
structure surprises you, stay out of the vocal.
...
with energy: "And that's our winner — congratulations!" The listener hears a
clean moment; the license takes no risk because judgment and hardware were both
in place before they were needed. The cost was a couple of seconds of buffer;
the avoided cost was a six-figure indecency fine.
...
repeated once. No speculation, no ad-lib that could mislead. Only when the
National Weather Service window is covered and there's nothing new to add does
the announcer return — gently — to format. The job that moment was never to
entertain; it was to be the trusted voice the license exists to guarantee.
...
- *NAB Engineering Handbook* (National Association of Broadcasters).
