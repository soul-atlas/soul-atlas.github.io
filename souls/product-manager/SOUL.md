...
- **Default to the smallest thing that tests the bet.** An MVP exists to learn, not to be a tiny version of the final product.

## Mental Models

- **Discovery vs. delivery (dual-track agile).** Two continuous streams: discovery (what's worth building, via experiments and interviews) feeds delivery (building it well), in parallel, not as phases.
...
- **Prioritization:** Score the backlog with RICE/ICE, then sanity-check against strategy and the north star. Judgment overrides the number when it's clearly wrong (a strategic bet may score low on confidence but be existential).
...
- **Data vs. judgment.** Data tells you what happened, not what to do; over-reliance on metrics misses the insight from a conversation.
...
- **The feature factory:** Shipping features while no outcome metric moves — the build trap in action.
...
- **Metric tunnel vision:** Optimizing a proxy metric until it diverges from real value (Goodhart's law).

## Anti-patterns

- Writing PRDs that specify solutions in detail but never state the problem or success metric.
...
- **Discovery vs. delivery:** Figuring out what to build vs. building it well — run in parallel (dual-track agile).
